  ...
  async create(input: CreateProjectInput): Promise<CreateProjectOutputDto> {
    ...
    await session
      ...
      .then(result => {
        ...
        response.project.languages = result.records[0].get('languages');
  ...
  async readOne(input: ReadProjectInput): Promise<ReadProjectOutputDto> {
    ...
    await session
      .run(
        ...
        WHERE project.id = $id
        RETURN project.id as id, project.name as name, project.deptId as deptId, project.status as status, project.location as location, project.publicLocation as publicLocation, project.mouStart as mouStart, project.mouEnd as mouEnd, project.languages as languages, project.partnerships as partnerships, project.sensitivity as sensitivity, project.team as team, project.budgets as budgets, project.estimatedSubmission as estimatedSubmission, project.engagements as engagements`,
        {
          id: input.id,
        },
      )
  ...
  async update(input: UpdateProjectInput): Promise<UpdateProjectOutputDto> {
    ...
    await session
      .run(
        ...
        SET project.email = $email, project.realFirstName = $realFirstName, project.realLastName = $realLastName, project.displayFirstName = $displayFirstName, project.displayLastName = $displayLastName
          RETURN project.id as id, project.name as name, project.deptId as deptId, project.status as status, project.location as location, project.publicLocation as publicLocation, project.mouStart as mouStart, project.mouEnd as mouEnd, project.languages as languages, project.partnerships as partnerships, project.sensitivity as sensitivity, project.team as team, project.budgets as budgets, project.estimatedSubmission as estimatedSubmission, project.engagements as engagements`,
        {
          id: input.id,
          name: input.name,
