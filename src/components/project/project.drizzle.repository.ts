...
> {
  ...
  override async readMany(
    ...
    // migration-todo (PR 4): when `changeset` is set, overlay
    // project_overrides.changes on top of the row's columns before toDto.
  ...
  async create(input: CreateProject): Promise<{ id: ID<'Project'> }> {
    ...
    // migration-todo (PR 3): `step` defaults to EarlyConversations via the
    // column default; SetInitialMouEnd handler runs on the Created hook.
    // SetDepartmentId runs on the Transitioned hook (PR 3 wires both).
    await this.db
      .insert(projects)
      .values({
        id,
  ...
  async update(
    ...
    // migration-todo (PR 4): when `changeset` is set, write into
    // `project_overrides.changes` JSONB instead of mutating the row.
    const {
      id: _id,
      changeset: _cs,
      primaryLocation,
  ...
  protected toDto(row: ProjectRow): UnsecuredDto<Project> {
    ...
    // yet (primaryPartnership, engagementTotal, usesRev79, changeset). Build
    // the dto as `unknown` first so the lint stays clean — service runs
    ...
    const dto: unknown = {
      ...
      // null until partnership-pg lands (stacks on PR 4).
      ...
      membership: row.membership
        ...
        : null,
      // migration-todo (PR 4): real changeset overlay. For now no changeset
      // view is supported under DATABASE=postgres.
      changeset: undefined,
