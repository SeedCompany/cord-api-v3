...
import { ProjectChangesetAfterFinalizedEvent } from '../../project-change-request/events';
import { ProjectStep } from '../dto';
import { ProjectRules } from '../project.rules';

type SubscribedEvent = ProjectChangesetAfterFinalizedEvent;

@EventsHandler(ProjectChangesetAfterFinalizedEvent)
export class AfterFinalizedChangesetToProject
  implements IEventHandler<SubscribedEvent>
{
  constructor(
    ...
    @Logger('project:change-request:after-finalized')
    private readonly logger: ILogger
  ) {}

  async handle(event: SubscribedEvent) {
    this.logger.debug('After finalized changeset');
