...
import { RoleChangeEvent } from '../events/role-change.event';
...
@EventsHandler(RoleChangeEvent)
export class UpdateMemberRoleHandler implements IEventHandler<RoleChangeEvent> {
  constructor(
  ...
  async handle(event: RoleChangeEvent) {
    let sgId;
