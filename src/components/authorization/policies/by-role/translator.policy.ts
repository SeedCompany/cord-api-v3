...
@Policy(Role.Translator, (r) => [
  ...
  r.Language.when(member).read.specifically(
    (p) => p.many('leastOfThese', 'leastOfTheseReason', 'sponsorEstimatedEndDate').none,
  ),
  ...
  r.ProgressReportWorkflowEvent.transitions('Translation Done', 'Translation Reject').execute,
