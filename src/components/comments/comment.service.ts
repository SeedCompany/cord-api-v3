...
import { CommentMentionedNotificationService } from './mentioned-notification/comment-mentioned-notification.service';
...
export class CommentService {
  constructor(
    ...
    private readonly mentionedService: CommentMentionedNotificationService,
  ) {}
  ...
  async create(input: CreateCommentInput, session: Session) {
    ...
    const mentionees = this.mentionedService.extract(dto);
    await this.mentionedService.notify(mentionees, dto);
  ...
  async update(input: UpdateCommentInput, session: Session): Promise<Comment> {
    ...
    const prevMentionees = this.mentionedService.extract(object);
    const nowMentionees = this.mentionedService.extract(updated);
    const newMentionees = difference(prevMentionees, nowMentionees);
    await this.mentionedService.notify(newMentionees, updated);
