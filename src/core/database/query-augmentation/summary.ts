...
Connection.prototype.executeAndReturnSummary = async function executeAndReturnSummary(
  this: Connection,
  query: Query,
) {
  if (!this.open) {
...
Query.prototype.executeAndReturnSummary = async function executeAndReturnSummary(this: Query) {
  if (!this.connection) {
...
Query.prototype.executeAndReturnStats = async function executeAndReturnStats(this: Query) {
  const summary = await this.executeAndReturnSummary();
  return summary.updateStatistics.updates();
};

Query.prototype.executeAndLogStats = async function executeAndLogStats(this: Query) {
  const summary = await this.executeAndReturnSummary();
  const stats = summary.updateStatistics.updates();
  const filteredStats = Object.fromEntries(Object.entries(stats).filter(([_, num]) => num > 0));
