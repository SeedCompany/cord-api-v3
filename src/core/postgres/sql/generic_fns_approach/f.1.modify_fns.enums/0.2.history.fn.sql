CREATE OR REPLACE procedure public.history_fn(pTableName text, pToggleHistory public.toggle_history, pRecord hstore)
LANGUAGE PLPGSQL
AS $$
declare 
...
begin
    if pToggleHistory = 'NoHistory' then 
        ...
        for rec1 in (select skeys(pRecord), svals(pRecord)) loop 
            if rec1.svals is not null then 
        ...
        raise info '% %', sql_string_keys, sql_string_values;
